...
namespace assignment1.Controllers
{
    ...
    public class q8 : ControllerBase
    {
        ...
        public string Squashfellows([FromForm] int Small, [FromForm] int Large)
        {
            ...
            decimal small_total = Small * 25.5m;
            decimal large_total = Large * 40.5m;
            decimal subtotal = small_total + large_total;
            decimal tax = Math.Round(subtotal * 0.13m,2, MidpointRounding.AwayFromZero);
            decimal total = Math.Round(subtotal*1.13m,2, MidpointRounding.AwayFromZero);
            // subtotal*1.13 = 516.9749999999999, no use for Math.Round, may due to floating-point precision problem when using double, use decimal to solve
            // decimal: all number comes with m
            // Math.Round = bankers' rounding, rounding to the nearest even number, can use MidpointRounding.AwayFromZero to solve
